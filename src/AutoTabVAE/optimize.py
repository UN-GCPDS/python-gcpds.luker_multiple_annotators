...
def suggest(trial, name, method, default, **kwargs):
    """
    Utility function to allow override of Optuna suggestions via config dict.
    """
    if method == "int":
...
def objective(trial, param_config=None):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    cfg = param_config or {}

    # Suggest hyperparameters (use config overrides if available)
...
def run_optuna(X, y, n_trials=50, param_config=None):
    """
    ...
    param_config : dict, optional
        Dictionary to customize the search space of hyperparameters.
    """
    objective.X = X
    objective.y = y

    study = optuna.create_study(direction="minimize")
    study.optimize(lambda trial: objective(trial, param_config), n_trials=n_trials)
    return study
